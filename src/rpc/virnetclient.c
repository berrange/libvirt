...
static gboolean
virNetClientIOEventTLS(GIOChannel *source,
                       GIOCondition ev,
                       gpointer opaque);
                         ...
                         GIOChannel *chan)
{
    ...
    g_io_add_watch(chan, ev, virNetClientIOEventTLS, client);
                       ...
                       gpointer opaque)
{
    ...
    if (!virNetClientTLSHandshake(client, source))
        g_main_loop_quit(client->eventLoop);
...
static gboolean
virNetClientIOEventTLSConfirm(GIOChannel *source G_GNUC_UNUSED,
                              GIOCondition ev G_GNUC_UNUSED,
                              gpointer opaque)
{
    virNetClientPtr client = opaque;
                              ...
                              virNetTLSContextPtr tls)
{
    ...
    char buf[1];
    int len;
    g_autoptr(GIOChannel) chan = NULL;
...
# ifdef WIN32
    chan = g_io_channel_win32_new_socket(
        _get_osfhandle(virNetSocketGetFD(client->sock)));
# else
    chan = g_io_channel_unix_new(virNetSocketGetFD(client->sock));
# endif
    ...
    virResetLastError();
    if (virNetClientTLSHandshake(client, chan)) {
# ifndef WIN32
...
# endif /* !WIN32 */
    ...
    g_io_add_watch(chan, G_IO_IN, virNetClientIOEventTLSConfirm, client);
...
static gboolean
virNetClientIOEventFD(GIOChannel *source G_GNUC_UNUSED,
                      GIOCondition ev,
                      gpointer opaque)
{
                                   ...
                                   virNetClientCallPtr thiscall)
{
    ...
    for (;;) {
        ...
        srcid = g_io_add_watch(chan, ev, virNetClientIOEventFD, &rev);
