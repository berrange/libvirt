                            ...
                            virDomainNetDefPtr iface)
{
    ...
    case VIR_NETWORK_FORWARD_ROUTE:
    case VIR_NETWORK_FORWARD_OPEN:
        port->type = VIR_NETWORK_PORT_TYPE_BRIDGE;

        if (VIR_STRDUP(port->data.bridge.brname, netdef->bridge) < 0)
            goto error;
        port->data.bridge.macTableManager = netdef->macTableManager;
    ...
    case VIR_NETWORK_FORWARD_HOSTDEV: {
        port->type = VIR_NETWORK_PORT_TYPE_HOSTDEV_PCI;
        ...
        port->data.hostdevpci.addr = dev->device.pci;
        port->data.hostdevpci.driver = netdef->forward.driverName;
        port->data.hostdevpci.managed = netdef->forward.managed;
    ...
    case VIR_NETWORK_FORWARD_PASSTHROUGH:
        ...
        port->type = VIR_NETWORK_PORT_TYPE_DIRECT;
        ...
        if ((netdef->forward.nifs <= 0) && (netdef->forward.npfs <= 0)) {
            ...
            if (VIR_STRDUP(port->data.direct.linkdev,
                           dev->device.dev) < 0)
                goto error;
        }
        break;
    ...
    if (port->vlan.nTags) {
         ...
         * fail
         */
        if (!(port->type == VIR_NETWORK_PORT_TYPE_HOSTDEV_PCI ||
              (port->type == VIR_NETWORK_PORT_TYPE_DIRECT &&
               port->data.direct.mode == VIR_NETDEV_MACVLAN_MODE_PASSTHRU) ||
              (port->type == VIR_DOMAIN_NET_TYPE_BRIDGE &&
               port->virtPortProfile &&
            ...
            if (netdef) {
                ...
                virReportError(VIR_ERR_CONFIG_UNSUPPORTED,
                               ...
                               virNetworkPortTypeToString(port->type));
            }
            goto error;
        }
    }
     ...
     */
    if (port->bandwidth && port->type == VIR_NETWORK_PORT_TYPE_HOSTDEV_PCI) {
        virReportError(VIR_ERR_CONFIG_UNSUPPORTED, "%s",
                           ...
                           virDomainNetDefPtr iface)
{
    ...
    switch ((virNetworkPortType)port->type) {
    ...
    case VIR_NETWORK_PORT_TYPE_DIRECT:
        ...
        for (i = 0; i < netdef->forward.nifs; i++) {
            if (netdef->forward.ifs[i].type
                == VIR_NETWORK_FORWARD_HOSTDEV_DEVICE_NETDEV &&
                STREQ(port->data.direct.linkdev, netdef->forward.ifs[i].device.dev)) {
                dev = &netdef->forward.ifs[i];
                break;
            }
        }
        ...
        if (!dev) {
            virReportError(VIR_ERR_INTERNAL_ERROR,
                           _("network '%s' doesn't have dev='%s' "
                             "in use by domain"),
                           netdef->name, port->data.direct.linkdev);
            goto cleanup;
        }
        break;
    ...
    case VIR_NETWORK_PORT_TYPE_HOSTDEV_PCI:
        ...
        for (i = 0; i < netdef->forward.nifs; i++) {
            if (netdef->forward.ifs[i].type
                == VIR_NETWORK_FORWARD_HOSTDEV_DEVICE_PCI &&
                virPCIDeviceAddressEqual(&port->data.hostdevpci.addr,
                                         &netdef->forward.ifs[i].device.pci)) {
                dev = &netdef->forward.ifs[i];
                break;
            }
        ...
        if (!dev) {
            virReportError(VIR_ERR_INTERNAL_ERROR,
                           _("network '%s' doesn't have "
                             "PCI device %04x:%02x:%02x.%x in use by domain"),
                           netdef->name,
                           port->data.hostdevpci.addr.domain,
                           port->data.hostdevpci.addr.bus,
                           port->data.hostdevpci.addr.slot,
                           port->data.hostdevpci.addr.function);
            goto cleanup;
        }
        break;
    ...
    default:
        virReportEnumRangeError(virNetworkPortType, port->type);
        goto cleanup;
    }
